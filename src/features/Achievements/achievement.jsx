  ...
  return (
    <div
      className={`achievement-badge ${
        isUnlocked ? `unlocked tier-${achievementInfo.tier}` : "locked"
      }`}
    >
      <div className="badge-icon-wrapper">
        {isUnlocked ? (
          <Icon size={48} className="main-badge-icon" />
        ) : (
          <Lock size={48} className="main-badge-icon" />
        )}
      </div>
...
function Achievement() {
  // Component chính để hiển thị danh sách thành tựu
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        if (!templatesRes.ok || !unlockedRes.ok)
          throw new Error("Lỗi tải dữ liệu thành tựu");

        const templateDtos = await templatesRes.json(); // chuyen tu API sang object
        const unlockedDtos = await unlockedRes.json(); // Lấy các achievement ĐÃ MỞ KHÓA từ unlockedRes

        const feTemplates = templateDtos.map(mapApiToFeAchievement); // them icon va tier từ mapper
        ...
        const knownCount = parseInt(
          localStorage.getItem(knownAchievementsKey) || "0",
          10
        );
        if (unlockedDtos.length > knownCount) {
          const knownAchievements = JSON.parse(
            localStorage.getItem(`known_achievements_list_${userId}`) || "[]"
          );
          const newAchievements = unlockedDtos.filter(
            (dto) => !knownAchievements.includes(dto.name)
          );
          newAchievements.forEach((newAchDto) => {
            const achievementTemplate = feTemplates.find(
              (a) => a.title === newAchDto.name
            );
        ...
        localStorage.setItem(
          knownAchievementsKey,
          unlockedDtos.length.toString()
        );
        localStorage.setItem(
          `known_achievements_list_${userId}`,
          JSON.stringify(unlockedDtos.map((dto) => dto.name))
        );
  ...
  const achievementsByCategory = {
    time: allAchievements.filter(
      (a) => a.category === "time" || a.category === "health"
    ),
    money: allAchievements.filter((a) => a.category === "money"),
    mission: allAchievements.filter(
      (a) => a.category === "mission" || a.category === "diary"
    ),
  };
    ...
    <>
      ...
      <div className="achievement-page">
        <header className="achievement-header">
          ...
          <p>
            Mỗi huy hiệu là một minh chứng cho sự kiên trì và nỗ lực phi thường
            của bạn!
          </p>
        </header>
