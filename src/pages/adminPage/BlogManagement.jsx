...
import { Plus, Search, Filter, FilePenLine, Trash2, Check, X } from 'lucide-react';
...
const StatusTag = ({ status }) => {
    ...
    return <span className={`status-tag ${currentStatus.className}`}>{currentStatus.text}</span>;
};
    ...
    const handleUpdateStatus = async (blogId, newStatus) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Không thể ${action === 'approve' ? 'phê duyệt' : 'từ chối'} bài viết.`);
            }
            ...
            if (newStatus === 'APPROVED') {
                setStats(prev => ({ ...prev, published: prev.published + 1, pending: prev.pending - 1 }));
            } else if (newStatus === 'REJECTED') {
                 setStats(prev => ({ ...prev, pending: prev.pending - 1 }));
            }

            alert(`Bài viết đã được ${newStatus === 'APPROVED' ? 'phê duyệt' : 'từ chối'} thành công!`);
    ...
    return (
        <div className="blog-management-page">
            ...
            <div className="stats-grid">
                <div className="card stat-card"><h3>{stats.total}</h3><p>Tổng bài đăng</p></div>
                <div className="card stat-card"><h3>{stats.published}</h3><p>Đã xuất bản</p></div>
                <div className="card stat-card"><h3>{stats.pending}</h3><p>Chờ duyệt</p></div>
                <div className="card stat-card"><h3>...</h3><p>Tổng lượt xem</p></div>
            </div>
            ...
            <div className="card">
                <div className="search-filter-bar">
                    ...
                    <div className="filter-buttons">
                        ...
                        <button className={`btn-filter ${filter === 'PENDING' ? 'active' : ''}`} onClick={() => setFilter('PENDING')}>Chờ duyệt</button>
                        <button className={`btn-filter ${filter === 'APPROVED' ? 'active' : ''}`} onClick={() => setFilter('APPROVED')}>Đã duyệt</button>
                        <button className={`btn-filter ${filter === 'REJECTED' ? 'active' : ''}`} onClick={() => setFilter('REJECTED')}>Bị từ chối</button>
                    </div>
                </div>
                
                <h3 className="list-title">Danh sách bài đăng ({filteredBlogs.length})</h3>
                    ...
                    {isLoading ? (
                        ...
                        filteredBlogs.map((blog) => (
                            <div className="table-row" key={blog.id}>
                                <div>{blog.title}</div>
                                <div>{blog.author?.name || 'Không xác định'}</div>
                                <div>{dayjs(blog.createdAt).format("DD/MM/YYYY")}</div>
                                <div><StatusTag status={blog.status} /></div>
                                <div className="action-buttons">
                                    ...
                                    <button className="btn-icon" title="Chỉnh sửa (tính năng tương lai)">
                                        <FilePenLine size={18} strokeWidth={1.5} />
                                    </button>
                                    <button className="btn-icon btn-icon-delete" title="Xóa (tính năng tương lai)">
